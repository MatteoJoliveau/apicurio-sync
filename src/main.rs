...
async fn run() -> Result<(), Error> {
    ...
    match opts.cmd.as_ref().unwrap_or(&Command::Sync {}) {
        ...
        Command::Sync => sync(&client_v2, &config, &lockfile, &workdir).await,
...
async fn update(provider: &impl Provider, config: &Config, lockfile: &mut LockFile) -> Result<(), Error> {
    eprintln!("Updating lockfile with remote registry at {}", config.url);
...
async fn sync(provider: &impl Provider, config: &Config, lockfile: &LockFile, workdir: &Path) -> Result<(), Error> {
    eprintln!("Syncing artifacts with remote registry at {}", config.url);
